...
event <- readr::read_csv(
  file.path(data_dir, "am_2017_01_v4_event_metadata.csv"), show_col_types = FALSE
) |>
  dplyr::slice(1) |>
  dplyr::mutate(
...
covariates <- readr::read_csv(
  file.path(data_dir, "am_2017_01_v4_covariates.csv"), show_col_types = FALSE
)
...
# V4: a single bimonthly panel holds all 8 outcomes.
fiscal_panel <- readr::read_csv(
  file.path(data_dir, "am_2017_01_v4_bimonthly_fiscal_panel.csv"), show_col_types = FALSE
) |> dplyr::mutate(period_date = as.Date(.data$period_date))
monthly_panel <- fiscal_panel  # alias for any legacy reference
...
# ── Specifications ─────────────────────────────────────────────────────────────
# V4: all 8 outcomes are bimonthly + STL. One family, one estimation spec.
specs <- list(
  list(
    data          = fiscal_panel,
    family        = "bimonthly",
...
readr::write_csv(summary_tbl, file.path(output_root, "am_2017_01_v4_scm_summary.csv"), na = "")

# ── LOO placebo ───────────────────────────────────────────────────────────────
placebo_specs <- list(
  list(data = fiscal_panel, family = "bimonthly", specification = "sa",
       weight_period = "pre",
...
message("AM 2017-01 V4 SCM completed.")
message("Saved: ", file.path(output_root, "am_2017_01_v4_scm_summary.csv"))
