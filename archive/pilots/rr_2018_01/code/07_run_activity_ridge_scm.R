...
output_dir <- file.path(pilot_root, "output", "activity_ridge_scm")
...
run_outcome <- function(outcome) {
  ...
  path_plot <- ggplot2::ggplot(fit$path, ggplot2::aes(x = period_date)) +
    ...
    ggplot2::labs(
      title = paste0("RR 2018 ridge synthetic: ", outcome),
      x = NULL,
      y = outcome,
      color = NULL
    ) +
...
activity_ridge_summary <- purrr::map_dfr(outcomes, run_outcome)

readr::write_csv(
  activity_ridge_summary,
  file.path(output_dir, "activity_ridge_summary.csv"),
  na = ""
)

message("RR 2018 activity ridge synthetic controls completed.")
