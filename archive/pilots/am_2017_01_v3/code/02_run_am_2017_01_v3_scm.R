...
event <- readr::read_csv(
  file.path(data_dir, "am_2017_01_v3_event_metadata.csv"), show_col_types = FALSE
) |>
  dplyr::slice(1) |>
  dplyr::mutate(
...
covariates <- readr::read_csv(
  file.path(data_dir, "am_2017_01_v3_covariates.csv"), show_col_types = FALSE
)
...
monthly_panel <- readr::read_csv(
  file.path(data_dir, "am_2017_01_v3_monthly_panel.csv"), show_col_types = FALSE
) |> dplyr::mutate(period_date = as.Date(.data$period_date))

fiscal_panel <- readr::read_csv(
  file.path(data_dir, "am_2017_01_v3_bimonthly_fiscal_panel.csv"), show_col_types = FALSE
) |> dplyr::mutate(period_date = as.Date(.data$period_date))
...
build_predictor_matrix <- function(data, outcome, donor_states, covariate_data,
  ...
  if (nrow(pre_data) < 6) stop("Too few complete pre-treatment periods: ", outcome)
...
fit_augmented_scm <- function(data, outcome, family, specification,
  ...
  if (length(treated_pre_dates) < 6) {
    return(tibble::tibble(
      ...
      status = "skipped",
      skip_reason = paste0("Insufficient pre-treatment data: ", outcome)
    ))
  }
...
run_loo_placebo <- function(data, outcome, family, specification, weight_period = "pre") {
  ...
  if (length(treated_pre_dates) < 6) return(NULL)
...
# ── Specifications ─────────────────────────────────────────────────────────────
# v2: SA (seasonally adjusted) replaces MA. Only one estimation spec per family.
specs <- list(
  list(
    data          = monthly_panel,
    family        = "monthly",
  ...
  list(
    ...
    weight_period = "pre",
    outcomes      = c(
      "state_tax_revenue_real_pc_sa",
      "icms_revenue_real_pc_sa",
      "public_investment_liquidated_real_pc_sa",
      "liquidated_expenditure_total_real_pc_sa"
    )
  )
)
...
readr::write_csv(summary_tbl, file.path(output_root, "am_2017_01_v3_scm_summary.csv"), na = "")

# ── LOO placebo ───────────────────────────────────────────────────────────────
placebo_specs <- list(
  list(data = monthly_panel,    family = "monthly",          specification = "sa",
       weight_period = "pre",
       outcomes = c("formal_hiring_balance_sa_per_100k_wap",
                    "retail_volume_index_sa", "services_volume_index_sa")),
  list(data = fiscal_panel,     family = "bimonthly_fiscal", specification = "sa",
       weight_period = "pre",
       outcomes = c("icms_revenue_real_pc_sa",
                    "public_investment_liquidated_real_pc_sa",
                    "liquidated_expenditure_total_real_pc_sa"))
)
...
message("AM 2017-01 V3 SCM completed.")
message("Saved: ", file.path(output_root, "am_2017_01_v3_scm_summary.csv"))
