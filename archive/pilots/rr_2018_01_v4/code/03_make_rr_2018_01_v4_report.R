...
outcome_meta <- tibble::tribble(
  ...
  "formal_hiring_balance_per_100k_wap_ma6_v4", "Formal hiring balance per 100k working-age population, MA6 V4", "Formal labor market", NA_character_, TRUE,
  "formal_hiring_balance_construction_per_100k_wap_ma6_v4", "Construction hiring balance per 100k working-age population, MA6 V4", "Formal labor market", NA_character_, TRUE,
  "retail_volume_index_ma6_v4", "Retail volume index, MA6 V4", "Household consumption", NA_character_, TRUE,
  "services_volume_index_ma6_v4", "Services volume index, MA6 V4", "Household consumption", NA_character_, TRUE,
  "state_tax_revenue_real_pc_ma4_v4", "Own tax revenue, real per capita, MA4 V4", "State public finances", "revenues", TRUE,
  "icms_revenue_real_pc_ma4_v4", "ICMS revenue, real per capita, MA4 V4", "State public finances", "revenues", TRUE,
  "public_investment_liquidated_real_pc_ma4_v4", "Public investment, liquidated, real per capita, MA4 V4", "State public finances", "expenditures", TRUE,
  "liquidated_expenditure_total_real_pc_ma4_v4", "Total liquidated expenditure, real per capita, MA4 V4", "State public finances", "expenditures", TRUE
)
...
icms_audit <- fiscal_panel |>
  ...
  dplyr::select(
    ...
    .data$icms_revenue_real_pc_ma4_v4
  )
...
total_expenditure_audit <- fiscal_panel |>
  ...
  dplyr::select(
    ...
    .data$liquidated_expenditure_total_real_pc_ma4_v4,
...
readr::write_csv(icms_audit, file.path(table_dir, "rr_2018_01_v4_icms_audit.csv"), na = "")
readr::write_csv(total_expenditure_audit, file.path(table_dir, "rr_2018_01_v4_total_expenditure_audit.csv"), na = "")
...
report_lines <- c(
  "# RR 2018-01 V4: clean pilot rebuild",
  "",
  ...
  "## Audit tables",
  "",
  "- ICMS audit: `report/tables/rr_2018_01_v4_icms_audit.csv`.",
  "- Total expenditure audit: `report/tables/rr_2018_01_v4_total_expenditure_audit.csv`.",
  "",
  "## Scope note",
  "",
  "This V4 pilot prioritizes clean reconstruction and internal consistency. It does not yet bundle placebo inference into the reporting layer. That step should come after the current specification is fully stabilized."
)

readr::write_lines(
  report_lines,
  file.path(pilot_root, "rr_2018_01_v4_results_report.md")
)
...
readr::write_lines(
  c(
    ...
    "- Uses the V4 smoothing rule: complete trailing windows in pre-treatment, expanding windows after the break.",
    "- Keeps the main post-treatment window inside 2019 to avoid pandemic overlap and the CAGED methodology break.",
    "",
    "## Scripts",
    "",
    "1. `code/01_build_rr_2018_01_v4_panels.R`",
    "2. `code/02_run_rr_2018_01_v4_scm.R`",
    "3. `code/03b_make_rr_2018_01_v4_report_figures.R`",
    "4. `code/03_make_rr_2018_01_v4_report.R`"
  ),
  file.path(pilot_root, "README.md")
)

message("RR 2018-01 V4 report completed.")
message("Saved report to: ", file.path(pilot_root, "rr_2018_01_v4_results_report.md"))
