...
event <- readr::read_csv(file.path(data_dir, "am_2017_01_v2_event_metadata.csv"), show_col_types = FALSE) |>
  dplyr::slice(1)

treated_st     <- event$state_abbrev[[1]]
state_name_val <- event$state_name[[1]]

fiscal_panel <- readr::read_csv(file.path(data_dir, "am_2017_01_v2_bimonthly_fiscal_panel.csv"), show_col_types = FALSE) |>
  dplyr::mutate(period_date = as.Date(.data$period_date))
quarterly_panel <- readr::read_csv(file.path(data_dir, "am_2017_01_v2_quarterly_pnadc_panel.csv"), show_col_types = FALSE) |>
  dplyr::mutate(period_date = as.Date(.data$period_date))
summary_tbl <- readr::read_csv(file.path(output_root, "am_2017_01_v2_scm_summary.csv"), show_col_types = FALSE)
...
outcome_meta <- tibble::tribble(
  ...
  "formal_hiring_balance_sa_per_100k_wap",              "Formal hiring balance per 100k WAP (SA)",       "Formal labor market",   "monthly",          "sa", "Formal hiring SA",
  "formal_hiring_balance_construction_sa_per_100k_wap", "Construction hiring per 100k WAP (SA)",         "Formal labor market",   "monthly",          "sa", "Construction SA",
  "retail_volume_index_sa",                             "Retail volume index (SA)",                      "Household consumption", "monthly",          "sa", "Retail SA",
  "services_volume_index_sa",                           "Services volume index (SA)",                    "Household consumption", "monthly",          "sa", "Services SA",
...
covariates_full <- readr::read_csv(file.path(data_dir, "am_2017_01_v2_covariates.csv"), show_col_types = FALSE)
...
loo_specs <- list(
  list(label = "Formal hiring SA",      slug = "formal_hiring_balance_sa_per_100k_wap",     family = "monthly",          spec = "sa"),
  list(label = "Retail SA",             slug = "retail_volume_index_sa",                    family = "monthly",          spec = "sa"),
  list(label = "Services SA",           slug = "services_volume_index_sa",                  family = "monthly",          spec = "sa"),
...
readr::write_csv(loo_results, file.path(table_dir, "am_2017_01_v2_loo_placebo_summary.csv"), na = "")
...
# ── Report ────────────────────────────────────────────────────────────────────
report_lines <- c(
  paste0("# ", pilot_label, " V2: results report (X-13 seasonal adjustment)"),
  "",
  paste0("Generated on ", Sys.Date(), "."),
  "",
  paste0("This version replaces moving-average smoothing with model-based seasonal adjustment for all sub-annual variables and covariates: X-13ARIMA-SEATS for monthly and quarterly series, and STL for bimonthly fiscal series (X-13 supports only quarterly and monthly frequencies). Because seasonality is removed at source, the analysis uses calendar time on the x-axis (not event-centered time). The three-segment logic — pre-treatment, crisis window, post-removal — is preserved."),
  "",
  "## Event design",
  "",
  paste0("- Treated state: `", treated_st, "` (", state_name_val, ")."),
  paste0("- Instability start: `", instability_dt, "` (first TRE/TSE cassation decision)."),
  paste0("- Effective removal: `", removal_dt, "` (TSE final cassation decision)."),
  paste0("- Crisis window: `", instability_dt, "` to `", removal_dt, "` (", event$instability_duration_days[[1]], " days)."),
  "",
  "## Window design",
  "",
  paste0("- Monthly: `", mon_ws, "` to `", mon_we, "`. Pre-treatment = 36 months; crisis window; post-removal = 12 months."),
  paste0("- Bimonthly fiscal: `", bim_ws, "` to `", bim_we, "`. Pre-treatment limited to 6 bimesters by Siconfi start (2015); crisis window; post-removal = 9 bimesters."),
...
readr::write_lines(report_lines, file.path(pilot_root, "am_2017_01_v2_results_report.md"))

run_summary_lines <- c(
  paste0("# ", pilot_label, " V2 Run Summary"),
  "",
...
message(pilot_label, " V2 report completed.")
message("Saved report to: ", file.path(pilot_root, "am_2017_01_v2_results_report.md"))
