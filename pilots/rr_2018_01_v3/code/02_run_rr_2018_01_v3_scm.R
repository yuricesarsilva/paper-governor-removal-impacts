...
plot_paths <- function(path, outcome, output_path) {
  ggplot2::ggplot(path, ggplot2::aes(x = .data$period_date)) +
    ...
    ggplot2::labs(
      title = paste0("RR 2018-01 V3: ", outcome),
...
plot_gaps <- function(path, outcome) {
  path |>
    ...
    ggplot2::labs(
      title = paste0("RR 2018-01 V3 gaps: ", outcome),
      subtitle = "Dotted line = instability start; dashed line = effective removal",
      x = NULL,
      y = "RR - Synthetic RR",
      color = NULL
...
run_visual_for_clean_outcome <- function(data, clean_outcome, visual_outcome, family, specification) {
  ...
  plot <- ggplot2::ggplot(plot_data, ggplot2::aes(x = .data$period_date)) +
    ...
    ggplot2::labs(
      title = paste0("RR 2018-01 V3 smoothed path: ", visual_outcome),
      subtitle = "Pre-treatment uses complete trailing windows; post-treatment restarts with expanding windows until the full window is reached",
      x = NULL,
      y = visual_outcome,
      color = NULL
    ) +
...
monthly_panel <- readr::read_csv(
  file.path(data_dir, "rr_2018_01_v3_monthly_panel.csv"),
  show_col_types = FALSE
) |>
...
quarterly_panel <- readr::read_csv(
  file.path(data_dir, "rr_2018_01_v3_quarterly_pnadc_panel.csv"),
  show_col_types = FALSE
) |>
  dplyr::mutate(period_date = as.Date(period_date))
...
readr::write_csv(summary, file.path(output_root, "rr_2018_01_v3_scm_summary.csv"), na = "")

message("RR 2018-01 V3 SCM completed.")
message("Saved summary to: ", file.path(output_root, "rr_2018_01_v3_scm_summary.csv"))
