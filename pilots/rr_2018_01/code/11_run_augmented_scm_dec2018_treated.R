...
output_dir <- file.path(pilot_root, "output", "augmented_scm_monthly_dec2018_treated")
post_clean_output_dir <- file.path(pilot_root, "output", "augmented_scm_monthly_dec2018_treated_post_clean")
...
pnadc <- read_processed("pnadc_sidra_quarterly_state_covariates_panel_ready.csv") |>
  dplyr::mutate(
    ...
    state_abbrev = as.character(state_abbrev),
    pnadc_transition_quarter = pnadc_period == "2018Q4"
  ) |>
  dplyr::select(
    pnadc_period,
    pnadc_period_date,
...
panel_base <- caged |>
  ...
  dplyr::mutate(
    ...
    donor_pool_main = !(state_abbrev %in% c("RR", "AM", "TO")),
    excluded_from_main_donor_pool = state_abbrev %in% c("RR", "AM", "TO"),
    transition_period = FALSE,
    pnadc_predictor_valid = pnadc_period != "2018Q4",
    analysis_period = dplyr::case_when(
      period_date >= as.Date("2016-01-01") & period_date <= as.Date("2018-11-01") ~ "pre",
      period_date >= as.Date("2018-12-01") & period_date <= as.Date("2020-12-01") ~ "post",
      TRUE ~ NA_character_
    ),
  ...
  dplyr::filter(!is.na(analysis_period)) |>
...
run_outcome <- function(data, output_path, outcome) {
  ...
  path_plot <- ggplot2::ggplot(fit$path, ggplot2::aes(x = period_date)) +
    ...
    ggplot2::geom_vline(xintercept = as.Date("2018-12-01"), linetype = "dashed", color = "gray35") +
...
message("RR 2018 augmented SCM monthly Dec-2018-treated robustness completed.")
