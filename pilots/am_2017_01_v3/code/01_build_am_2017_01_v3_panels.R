...
# output: "sa"    -> seasonally adjusted series (trend + irregular); removes seasonality only
#         "trend" -> trend-cycle component (Henderson filter via X-13, STL trend for freq 6);
#                    removes BOTH seasonality and high-frequency irregular noise.
# V3 uses output="trend" for the volatile monthly outcomes (labor/consumption) and
# keeps output="sa" for quarterly covariates and bimonthly fiscal.
x13_adjust <- function(x, dates, freq, state_id = "", output = "sa") {
  ...
  adjusted <- tryCatch({
    if (freq %in% c(12L, 4L)) {
      ...
      # ARIMA forecast extension stabilizes the Henderson trend at the endpoints.
      fit <- seasonal::seas(ts_obj, transform.function = "none", x11 = "", outlier = NULL)
      sa  <- if (output == "trend") {
        as.numeric(seasonal::trend(fit))   # trend-cycle (X-11 D12)
      } else {
        as.numeric(seasonal::final(fit))   # seasonally adjusted
      }
    } else {
      # X-13 cannot process freq != 4, 12 (e.g. bimonthly = 6). STL fallback.
      fit <- stats::stl(ts_obj, s.window = "periodic", robust = TRUE)
      sa  <- if (output == "trend") {
        as.numeric(fit$time.series[, "trend"])                       # STL trend-cycle
      } else {
        as.numeric(ts_obj) - as.numeric(fit$time.series[, "seasonal"]) # seasonally adjusted
      }
    }
...
# Apply X-13/STL to a long-format panel (one column per variable).
# Returns the panel with additional *_sa columns (named _sa for pipeline
# compatibility regardless of output mode; see x13_adjust for what _sa holds).
apply_x13_to_panel <- function(data, vars, date_col, group_col, freq, output = "sa") {
  message("Applying ", if (freq %in% c(12L, 4L)) "X-13" else "STL",
          " (freq=", freq, ", output=", output, ") to: ", paste(vars, collapse = ", "))
  data |>
    ...
    dplyr::mutate(
      dplyr::across(
        ...
        function(col_vals) {
          ...
          x13_adjust(col_vals, .data[[date_col]], freq, state_id, output)
        },
        .names = "{.col}_sa"
      )
    ) |>
...
message("--- Seasonal adjustment / trend-cycle extraction ---")

# Monthly OUTCOMES (freq = 12): V3 uses X-13 TREND-CYCLE (output="trend") to remove
# both seasonality and the high-frequency irregular noise that made labor/consumption
# series too volatile to read in V2 (which used the seasonally-adjusted series).
caged_sa <- caged_full |>
  dplyr::select(.data$state_abbrev, .data$period_date, .data$formal_hiring_balance) |>
  apply_x13_to_panel("formal_hiring_balance", "period_date", "state_abbrev", 12L, output = "trend")

construction_sa <- construction_full |>
  dplyr::select(.data$state_abbrev, .data$period_date, .data$formal_hiring_balance_construction) |>
  apply_x13_to_panel("formal_hiring_balance_construction", "period_date", "state_abbrev", 12L, output = "trend")

# PMC/PMS are already indices; X-13 trend-cycle applied to the index level directly.
retail_sa <- retail_full |>
  dplyr::select(.data$state_abbrev, .data$period_date, .data$retail_volume_index) |>
  apply_x13_to_panel("retail_volume_index", "period_date", "state_abbrev", 12L, output = "trend")

services_sa <- services_full |>
  dplyr::select(.data$state_abbrev, .data$period_date, .data$services_volume_index) |>
  apply_x13_to_panel("services_volume_index", "period_date", "state_abbrev", 12L, output = "trend")
...
event_metadata <- event |>
  dplyr::mutate(
    ...
    sa_method                    = "monthly outcomes: X-13 trend-cycle; quarterly covariates: X-13 SA; bimonthly fiscal: STL SA"
  )

# ── Write outputs ─────────────────────────────────────────────────────────────
readr::write_csv(monthly_panel,   file.path(pilot_data_dir, "am_2017_01_v3_monthly_panel.csv"),           na = "")
readr::write_csv(fiscal_panel,    file.path(pilot_data_dir, "am_2017_01_v3_bimonthly_fiscal_panel.csv"),  na = "")
readr::write_csv(quarterly_panel, file.path(pilot_data_dir, "am_2017_01_v3_quarterly_pnadc_panel.csv"),   na = "")
readr::write_csv(covariates,      file.path(pilot_data_dir, "am_2017_01_v3_covariates.csv"),              na = "")
readr::write_csv(event_metadata,  file.path(pilot_data_dir, "am_2017_01_v3_event_metadata.csv"),          na = "")

message("AM 2017-01 V3 panels built (X-13 trend-cycle for monthly):")
