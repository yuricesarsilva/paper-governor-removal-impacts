...
covariates_raw    <- readr::read_csv(file.path(data_dir, "am_2017_01_v3_covariates.csv"),             show_col_types = FALSE)
monthly_panel     <- readr::read_csv(file.path(data_dir, "am_2017_01_v3_monthly_panel.csv"),          show_col_types = FALSE) |> dplyr::mutate(period_date = as.Date(.data$period_date))
fiscal_panel      <- readr::read_csv(file.path(data_dir, "am_2017_01_v3_bimonthly_fiscal_panel.csv"), show_col_types = FALSE) |> dplyr::mutate(period_date = as.Date(.data$period_date))
...
# V3: monthly channels use X-13 trend-cycle; fiscal uses STL seasonally-adjusted.
method_label <- function(channel_slug) {
  if (channel_slug == "public_sector") "SA" else "trend-cycle"
}

# ── Outcome meta ──────────────────────────────────────────────────────────────
# v2: SA specification only; x-axis = period_date for all plots
meta <- tibble::tribble(
  ~channel_slug,  ~channel_label,          ~family,            ~specification, ~outcome,                                              ~short_label,
  "labor_market", "Formal labor market",   "monthly",          "sa", "formal_hiring_balance_sa_per_100k_wap",              "Formal hiring TC",
  "labor_market", "Formal labor market",   "monthly",          "sa", "formal_hiring_balance_construction_sa_per_100k_wap", "Construction TC",
  "consumption",  "Household consumption", "monthly",          "sa", "retail_volume_index_sa",                             "Retail TC",
  "consumption",  "Household consumption", "monthly",          "sa", "services_volume_index_sa",                           "Services TC",
  "public_sector","State public finances", "bimonthly_fiscal", "sa", "state_tax_revenue_real_pc_sa",                       "Own tax revenue SA",
  "public_sector","State public finances", "bimonthly_fiscal", "sa", "icms_revenue_real_pc_sa",                            "ICMS SA",
  "public_sector","State public finances", "bimonthly_fiscal", "sa", "public_investment_liquidated_real_pc_sa",             "Public investment SA",
  "public_sector","State public finances", "bimonthly_fiscal", "sa", "liquidated_expenditure_total_real_pc_sa",             "Total expenditure SA"
)
...
build_preliminary_plot <- function(channel_slug_value) {
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::labs(
      title    = paste0("Preliminary view (", method_label(channel_slug_value), "): ", unique(mr$channel_label)),
...
build_augmented_paths_plot <- function(channel_slug_value) {
  ...
  ggplot2::ggplot(path_long, ggplot2::aes(x = .data$period_date, y = .data$value, color = .data$series)) +
    ...
    ggplot2::facet_wrap(~short_label, scales = "free_y", ncol = 2) +
    ggplot2::labs(
      title    = paste0("Augmented SCM paths (", method_label(channel_slug_value), "): ", unique(mr$channel_label)),
...
build_augmented_gaps_plot <- function(channel_slug_value) {
  ...
  ggplot2::ggplot(gap_data, ggplot2::aes(x = .data$period_date, y = .data$augmented_gap)) +
    ...
    ggplot2::facet_wrap(~short_label, scales = "free_y", ncol = 2) +
    ggplot2::labs(
      title    = paste0("Augmented SCM gaps (", method_label(channel_slug_value), "): ", unique(mr$channel_label)),
...
build_weight_plot <- function(channel_slug_value) {
  ...
  ggplot2::ggplot(weight_data, ggplot2::aes(x = .data$scm_weight, y = .data$donor_state)) +
    ...
    ggplot2::labs(title = paste0("Donor weights (", method_label(channel_slug_value), "): ", unique(mr$channel_label)), x = "Weight", y = NULL) +
    base_theme()
}
...
message("AM 2017-01 V3 report figures regenerated.")
