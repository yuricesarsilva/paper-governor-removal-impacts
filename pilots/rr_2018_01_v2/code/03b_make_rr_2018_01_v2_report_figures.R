...
event <- readr::read_csv(file.path(data_dir, "rr_2018_01_v2_event_metadata.csv"), show_col_types = FALSE) |>
  dplyr::slice(1) |>
  dplyr::mutate(
...
covariates_raw <- readr::read_csv(file.path(data_dir, "rr_2018_01_v2_covariates.csv"), show_col_types = FALSE)
main_donor_states <- covariates_raw |>
  dplyr::filter(.data$donor_pool_main) |>
  dplyr::pull(.data$state_abbrev) |>
  sort()

monthly_panel <- readr::read_csv(file.path(data_dir, "rr_2018_01_v2_monthly_panel.csv"), show_col_types = FALSE) |>
  dplyr::mutate(period_date = as.Date(period_date))

fiscal_panel <- readr::read_csv(file.path(data_dir, "rr_2018_01_v2_bimonthly_fiscal_panel.csv"), show_col_types = FALSE) |>
  dplyr::mutate(period_date = as.Date(period_date))
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::labs(
      ...
      subtitle = if (specification_value == "raw") {
        ...
        "Linhas cinza = doadores elegiveis; medias moveis visuais usam janelas parciais no inicio de cada segmento"
      },
      x = NULL,
      y = NULL,
      color = NULL
...
message("RR 2018-01 V2 report figures regenerated with stable ASCII labels.")
